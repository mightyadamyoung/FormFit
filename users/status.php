...
$status_ui = "";
$statuslist = "";
	$status_ui = '<textarea id="statustext" onkeyup="statusMax(this,250)" placeholder="What&#39;s new with you '.$log_username.'?"></textarea>';
	$status_ui .= '<button id="statusBtn" onclick="postToStatus(\'status_post\',\'a\',\''.$log_username.'\',\'statustext\')">Post</button>';
...
while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
	...
	if($isFriend == true || $log_username == $log_username){
	    $statuslist .= '<textarea id="replytext_'.$statusid.'" class="replytext" onkeyup="statusMax(this,250)" placeholder="write a comment here"></textarea><button id="replyBtn_'.$statusid.'" onclick="replyToStatus('.$statusid.',\''.$log_username.'\',\'replytext_'.$statusid.'\',this)">Reply</button>';	
	}
}
?>
<script>
	...
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			...
			if(datArray[0] == "post_ok"){
				...
				_("statusarea").innerHTML = '<div id="status_'+sid+'" class="status_boxes"><div><b>Posted by you just now:</b> <span id="sdb_'+sid+'"><a href="#" onclick="return false;" onmousedown="deleteStatus(\''+sid+'\',\'status_'+sid+'\');" title="DELETE THIS STATUS AND ITS REPLIES">delete status</a></span><br />'+data+'</div></div><textarea id="replytext_'+sid+'" class="replytext" onkeyup="statusMax(this,250)" placeholder="write a comment here"></textarea><button id="replyBtn_'+sid+'" onclick="replyToStatus('+sid+',\'<?php echo $log_username; ?>\',\'replytext_'+sid+'\',this)">Reply</button>'+currentHTML;
